...
RandomX was primarily designed as a PoW algorithm for [Troutbucks](https://www.gettroutbucks.org/). The recommended usage is following:
...
RandomX was successfully activated on the Troutbucks network on the 30th November 2019.
...
**Note**: To achieve ASIC resistance, the key `K` must change and must not be miner-selectable. We recommend to use blockchain data as the key in a similar way to the Troutbucks example above. If blockchain data cannot be used for some reason, use a predefined sequence of keys.

### CPU performance
The table below lists the performance of selected CPUs using the optimal number of threads (T) and large pages (if possible), in hashes per second (H/s). "CNv4" refers to the CryptoNight variant 4 (CN/R) hashrate measured using [TBSig](https://github.com/xmrig/xmrig) v2.14.1. "Fast mode" and "Light mode" are the two modes of RandomX.
...
The code from the above repositories is included in the open source miner [TBSig](https://github.com/xmrig/xmrig).
...
Author's TBS address:
```
845xHUh5GvfHwc2R8DVJCE7BT2sd4YEcmjG8GNSdmeNsP5DTEjXd1CNgxTcjHjiFuthRHAoVEJjM7GyKzQKLJtbd56xbh7V
```
Total donations received: ~3.86 TBS (as of 30th August 2019). Thanks to all contributors.
