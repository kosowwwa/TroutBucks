...
gsigs = 'https://github.com/troutbucks-project/gitian.sigs.git'
...
def setup():
    ...
    os.chdir('inputs')
    if not os.path.isdir('troutbucks'):
        subprocess.check_call(['git', 'clone', args.url, 'troutbucks'])
    os.chdir('..')
...
def rebuild():
    ...
    for i in args.os:
        ...
        infile = 'inputs/troutbucks/contrib/gitian/gitian-' + tag_name + '.yml'
        subprocess.check_call(['bin/gbuild', '-j', args.jobs, '-m', args.memory, '--commit', 'troutbucks='+args.commit, '--url', 'troutbucks='+args.url, infile])
        ...
        subprocess.check_call('mv build/out/troutbucks-*.' + suffix + ' ../out/'+args.version, shell=True)
...
def build():
    ...
    subprocess.check_call(['make', '-C', 'inputs/troutbucks/contrib/depends', 'download', 'SOURCES_PATH=' + os.getcwd() + '/cache/common'])
...
def verify():
    ...
    for i, v in platforms:
        ...
        subprocess.check_call(['bin/gverify', '-v', '-d', '../sigs/', '-r', args.version+'-'+v[1], 'inputs/troutbucks/contrib/gitian/gitian-'+v[1]+'.yml'])
    os.chdir(workdir)
...
def main():
    ...
    parser.add_argument('-u', '--url', dest='url', default='https://github.com/troutbucks-project/troutbucks', help='Specify the URL of the repository. Default is %(default)s')
    ...
    os.makedirs('builder/inputs/troutbucks', exist_ok=True)
    os.chdir('builder/inputs/troutbucks')
    if args.pull:
