...
*Setup instructions for a Gitian build of Troutbucks.*

Gitian is the deterministic build process that is used to build the Troutbucks CLI
...
```bash
cp troutbucks/contrib/gitian/gitian-build.py .
```
...
fork the [gitian.sigs repository](https://github.com/troutbucks-project/gitian.sigs) and clone it on your host machine, 
or pass the signed assert file back to your build machine.

```bash
git clone git@github.com:troutbucks-project/gitian.sigs.git
...
gpg --detach-sign ${VERSION}-linux/${GH_USER}/troutbucks-linux-*-build.assert
gpg --detach-sign ${VERSION}-win/${GH_USER}/troutbucks-win-*-build.assert
gpg --detach-sign ${VERSION}-osx/${GH_USER}/troutbucks-osx-*-build.assert
gpg --detach-sign ${VERSION}-android/${GH_USER}/troutbucks-android-*-build.assert
gpg --detach-sign ${VERSION}-freebsd/${GH_USER}/troutbucks-freebsd-*-build.assert
```
...
[troutbucks-project/gitian.sigs](https://github.com/troutbucks-project/gitian.sigs/) repository:
...
**Note:** Please ensure your gpg public key is available to check signatures by adding it to the [gitian.sigs/gitian-pubkeys/](https://github.com/troutbucks-project/gitian.sigs/tree/master/gitian-pubkeys) directory in a pull request.
...
```bash
./gitian-build.py --detach-sign --no-commit --url https://github.com/troutbucksmooo-troutbucks/bittroutbucks -b troutbucksmooo 1f5680c8db8f4cc7acc04a04c724b832003440fd
```
