# The Current/Future Status of ZMQ in Troutbucks

## ZMQ Pub/Sub
Client `ZMQ_SUB` sockets must "subscribe" to topics before it receives any data.
This allows filtering on the server side, so network traffic is reduced. Troutbucks
...
matching supported by both Troutbucks and ZMQ. The `format`, `context` and `event`
...
The Troutbucks daemon will ensure that events prefixed by `chain` will be sent in
...
chain instead of the last block. The Troutbucks daemon also ensures that
...
The Troutbucks daemon will send a `txpool_add` pub exactly once for each
