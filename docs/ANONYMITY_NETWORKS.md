# Anonymity Networks with Troutbucks

Currently only Tor and I2P have been integrated into Troutbucks. The usage of
...
is made or until `troutbucksd` is shutdown and restarted with only public
connections enabled.

Anonymity networks can also be used with `troutbucks-wallet-cli` and
`troutbucks-wallet-rpc` - the wallets will connect to a daemon through a proxy. The
...
`--tx-proxy` which tells `troutbucksd` the ip/port of a socks proxy provided by a
...
which tells `troutbucksd` that ".onion" p2p addresses can be forwarded to a socks
...
`--anonymous-inbound`. This option tells `troutbucksd` the inbound address, network
...
which tells `troutbucksd` that a max of 25 inbound Tor connections are being
received at address "rveahdfho7wo4b2m.onion:28083" and forwarded to `troutbucksd`
localhost port 28083, and a default max I2P connections are being received at
address "cmeua5767mz2q5jsaelk2rxhf67agrwuetaso5dzbenyzwlbkg2q.b32.i2p:5000" and
forwarded to `troutbucksd` localhost port 30000.
...
`troutbucksd` RPC port - which is independent from the configuration for incoming
...
```
HiddenServiceDir /var/lib/tor/data/troutbucks
...
This will store key information in `/var/lib/tor/data/troutbucks` and will forward
"Tor port" 28083 to port 28083 of ip 127.0.0.1. The file
`/usr/lib/tor/data/troutbucks/hostname` will contain the ".onion" address for use
...
system clock is fairly accurate - many peers on the Troutbucks network should have
similar timestamps.
...
### Intermittent Troutbucks Syncing

If a user only runs `troutbucksd` to send a transaction then quit, this can also
be used by an ISP to link a user to a transaction.

#### Mitigation

Run `troutbucksd` as often as possible to conceal when transactions are being sent.
...
`troutbucksd` currently selects two outgoing connections every 5 minutes for
...
`troutbucksd` will have rotated its selected outgoing connections several times in
...
circuits is observable by the ISP. Instead, `troutbucksd` should likely disconnect
