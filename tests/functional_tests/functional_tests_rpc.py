...
# a main offline troutbucksd, does most of the tests
# a restricted RPC troutbucksd setup with RPC payment
# two local online troutbucksds connected to each other
N_TROUTBUCKSDS = 4

# 4 wallets connected to the main offline troutbucksd
# a wallet connected to the first local online troutbucksd
N_WALLETS = 5

WALLET_DIRECTORY = builddir + "/functional-tests-directory"
DIFFICULTY = 10

troutbucksd_base = [builddir + "/bin/troutbucksd", "--regtest", "--fixed-difficulty", str(DIFFICULTY), "--no-igd", "--p2p-bind-port", "troutbucksd_p2p_port", "--rpc-bind-port", "troutbucksd_rpc_port", "--zmq-rpc-bind-port", "troutbucksd_zmq_port", "--non-interactive", "--disable-dns-checkpoints", "--check-updates", "disabled", "--rpc-ssl", "disabled", "--data-dir", "troutbucksd_data_dir", "--log-level", "1"]
troutbucksd_extra = [
  ["--offline"],
...
wallet_base = [builddir + "/bin/troutbucks-wallet-rpc", "--wallet-dir", WALLET_DIRECTORY, "--rpc-bind-port", "wallet_port", "--disable-rpc-login", "--rpc-ssl", "disabled", "--daemon-ssl", "disabled", "--log-level", "1"]
wallet_extra = [
  ["--daemon-port", "18180"],
  ["--daemon-port", "18180"],
  ["--daemon-port", "18180"],
...
for i in range(N_TROUTBUCKSDS):
  command_lines.append([str(18180+i) if x == "troutbucksd_rpc_port" else str(18280+i) if x == "troutbucksd_p2p_port" else str(18380+i) if x == "troutbucksd_zmq_port" else builddir + "/functional-tests-directory/troutbucksd" + str(i) if x == "troutbucksd_data_dir" else x for x in troutbucksd_base])
  if i < len(troutbucksd_extra):
    command_lines[-1] += troutbucksd_extra[i]
  outputs.append(open(builddir + '/tests/functional_tests/troutbucksd' + str(i) + '.log', 'a+'))
