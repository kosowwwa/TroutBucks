...
Core tests take longer than any other Troutbucks tests, due to the high amount of computational work involved in validating core components.
...
To run only Troutbucks's core tests (after building):
...
To run only Troutbucks's crypto tests (after building):
...
```bash
troutbucksd --regtest --offline --fixed-difficulty 1
```
...
Open the wallet file with `troutbucks-wallet-rpc` with RPC port 18083. Finally, start tests by invoking ./blockchain.py or ./speed.py
...
To run only Troutbucks's hash tests (after building):
...
To run only Troutbucks's performance tests (after building):
...
To run only Troutbucks's unit tests (after building):
